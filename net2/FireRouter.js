...
class FireRouter {
  ...
  async init(first = false) {
    await lock.acquire(LOCK_INIT, async () => {
        ...
        if (routerConfig && routerConfig.routing && routerConfig.routing.global && routerConfig.routing.global.default) {
          ...
          switch (defaultRoutingConfig.type) {
            case Constants.WAN_TYPE_FAILOVER: {
              ...
              defaultWanIntfName = primaryWanIntfName;
              break;
            }
            case Constants.WAN_TYPE_LOAD_BALANCE: {
              const viaIntfs = defaultRoutingConfig.viaIntfs || [defaultRoutingConfig.viaIntf, defaultRoutingConfig.viaIntf2].filter(Boolean);
              primaryWanIntfName = viaIntfs[0] || null;
              for (const viaIntf of viaIntfs) {
                routingWans.push(viaIntf);
              }
              defaultWanIntfName = primaryWanIntfName;
            ...
            default: {
              ...
              if (defaultWanIntfName)
                routingWans.push(defaultWanIntfName);
              break;
            }
          }
        }

        monitoringIntfNames = safeCheckMonitoringInterfaces(routerConfig.monitoring && routerConfig.monitoring.interfaces || []);
        logicIntfNames = routerConfig.monitoring && routerConfig.monitoring.logicalInterfaces || [];
        networkInfos = await generateNetworkInfo()

        // save to sys:network:info after initialization
        this.sysNetworkInfo = networkInfos;

        if (first) {
          sem.once("release_ready", () => {
            this.ready = true;
          });
        }

        if (lastMonitoringIntfNames.length > 0 && monitoringIntfNames.length > 0) {
          const changedMonitoringIntfNames = _.xor(lastMonitoringIntfNames, monitoringIntfNames);
          if (changedMonitoringIntfNames.length > 0) {
            sem.emitEvent({type: "Message", message: Message.MSG_SYS_NETWORK_INFO_UPDATED});
          }
        }
      } else {
        // red, blue
        networkInfos = [];
        const nl = new (require('./NetworkTool.js'))();
        networkInfos = await nl.listInterfaces()
        if (f.isMain()) {
          await rclient.delAsync('sys:network:info');
          await rclient.delAsync('sys:network:uuid');
          for (const i of networkInfos) {
            await rclient.hsetAsync('sys:network:info', i.name, JSON.stringify(i))
            await rclient.hsetAsync('sys:network:uuid', i.uuid, JSON.stringify(i))
          }
          await pclient.publishAsync(Message.MSG_SYS_NETWORK_INFO_UPDATED, "");
        }
        this.sysNetworkInfo = networkInfos;
      ...
      if (!platform.isFireRouterManaged()) {
        // non-firerouter managed should use old network monitoring interface
        await this.updateMonitoringInterface()
        const redisStatus = await this.getNetworkStatus();
        if (!redisStatus)
          await this.resetMonitoringInterface()
      }
      
      if (f.isMain())
        await this.processPrimaryInterface();
  ...
  async resetMonitoringInterface() {
    // ...
  }

  async processPrimaryInterface() {
    // ...
  }

  async scheduleRestartPcap() {
    // ...
  }

  async configureTC(iface, qosNetworkType = "lan") {
  ...
  async removeFile(filename) {
    ...
    const resp = await rp(options)
    if (resp.statusCode !== 200) {
      throw new Error(`Error remove file ${filename}: ${resp.body}`);
    }
    return resp.body;
  }
