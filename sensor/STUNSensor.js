...
class STUNSensor extends Sensor {
  ...
  async apiRun() {
    extensionManager.onCmd("nat_type_check", async (msg, data) => {
      ...
      for (const stunServer of servers) {
        log.info(`Trying stun server ${stunServer.host}:${stunServer.port} ...`);
        let dstIP = new Address4(stunServer.host).isValid() ? stunServer.host : await exec(`dig +short +time=2 +tries=2 A ${stunServer.host} | tail -1`).then((result) => result.stdout.trim()).catch((err) => null);
        if (!dstIP) {
          log.error(`Cannot resolve stun server address ${stunServer}`);
          continue;
        }
        const natType = await this.detectNatType(Buffer.from(uuid.parse(uuid.v4())), sourceIP, dstIP, stunServer.port);
        if (natType === NAT_TYPE_UNKNOWN) {
          log.error(`Cannot determine NAT type using STUN server ${stunServer.host}:${stunServer.port}`);
          continue;
        }
        return natType;
      }
      log.error(`Cannot determine NAT type after trying all STUN servers`);
