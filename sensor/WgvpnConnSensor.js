...
class WgvpnConnSensor extends Sensor {
  ...
  async _checkWgPeersActivity() {
    ...
    if (_.isArray(peers)) {
      ...
      const results = await exec(`sudo wg show all latest-handshakes`).then(result => result.stdout.split('\n')).catch((err) => {
      ...
      for (const result of results) {
        ...
        if (Number(latestHandshake) > Date.now() / 1000 - CHECK_INTERVAL) {
          ...
          let remoteIP = null;
          let remotePort = null;
          const endpointsResults = (await exec(`sudo wg show ${intf} endpoints`).then(result => result.stdout.split('\n')).catch((err) => {
