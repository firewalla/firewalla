...
class FlowCompressionSensor extends Sensor {
  constructor() {
    super()
    this.count = (this.config && this.config.count * platform.getCompresseCountMultiplier()) || 10000
  ...
  async build(now) {
    ...
    try {
      ...
      for (let i = 0; i < (end - begin) / this.step; i++) {
        ...
        const endTs = begin + this.step * (i + 1)
        await this.loadFlows(beginTs, endTs, now > endTs) // only update the lastesTs for the entire hourly flows loaded
      }
      await this.checkAndCleanMem()
  ...
  async loadFlows(begin, end, updateTs) {
    ...
    while (!completed) {
      try {
        ...
        if (allFlows.length >= this.count) {
          // compress and dump flows to redis if it exceed count 
          // big array of allFlows might cause oom
          await this.appendAndSave(end, await this.compress(allFlows), updateTs)
          allFlows = [];
        }
      } catch (e) {
