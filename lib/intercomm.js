...
class InterComm {
  constructor(opts = {}) {
        if (!instance) {
            instance = this;
            this.bonjour = require('../vendor_lib/bonjour')(opts);
    ...
    encode_uuid(uid) {
        let buff = Buffer.from(uuid.parse(uid));
                ...
                if (service) {
                    service.stop(() => {
                        setTimeout(() => {
                            ...
                            service.on('error', (err) => log.error("Error re-publishing", serviceName, err));
                    ...
                    service.on('error', (err) => log.error("Error publishing", serviceName, err));
                    this.serviceMap[serviceName] = service;
                    return service;
                }
            } catch (e) {
...
module.exports = (opts = {}) => new InterComm(opts);
