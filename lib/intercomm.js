...
var intercomm = new class {
    ...
    publishInfo(gid, eid, gname, device, location, info, medium) {
        try {
            ...
            console.log("Publishing Info error ", txt);
        }
    }
    ...
    bpublish(gid, rid, typecode) {
        console.log("XX BlueTooth Publishing Started", gid, rid);
        var bleno = require('bleno');
        bleno.on('stateChange', (state) => {
            bleno.stopAdvertising();
            console.log("XX Bluetooth state", state);
            if (state == 'poweredOn') {
                let name = "fb" + typecode + this.encode_uuid(gid); //+gid.splice(19,-1);
                console.log("BlueTooth ", state, "publishing ", name, rid);
    ...
    unpublish(serviceName, callback) {
        ...
        if (service != null) {
            console.log("Unpubishing all");
            service.stop(() => {
                console.log("### UNPIUBLSIHED", serviceName);
    ...
    publish(medium, name, type, port, protocol, txt) {
        ...
        this.unpublish(serviceName, () => {
            try {
                ...
                console.log("Publishing error ", options);
            }
        });
    }
    ...
    discover(medium, type, callback) {
        this.bonjour.find({
            ...
            if (service.name.startsWith('eph:')) {
                ...
                if (array[1] == "devhi") {
                    console.log(service.name, service.txt);
                } else if (array[1] == 'devinfo') {
                    console.log(array[1], service.name, service.txt);
                }
