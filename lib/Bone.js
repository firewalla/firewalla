...
const endpoint = 'https://firewalla.encipher.io/bone/api/dv2';
...
let intelFeedback = function(intel, callback) {
  let options = {
...
exports.submitIntelFeedback = function (action, intel, data_type) {
  ...
  log.info("Submit Intel Feedback: '%s' => ", action, intel, {});
  ...
  let type = intel['if.type'];
  let target = intel['if.target'];
  ...
  switch (type) {
    case 'dns':
      _value = flowUtil.hashHost(target);
      break;
    case 'ip':
      _value = flowUtil.hashIp(target);
      break;
    default:
  }

  log.info("Action:", action, "Type:", type, ", Original value:", target, ", Hashed value:", _value,  {});
  ...
  intelFeedback(feedback, (err) => {
    if (err) {
      ...
      log.error("Submit intel feedback successfully");
    }
  });
