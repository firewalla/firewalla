...
let features = require('../config/features.js');
...
exports.intel = function(type, ip, action, obj, callback) {
  log.debug("/intel/" + type + "/" + ip + "/" + action);
  let options = {
    uri: getEndpoint() + '/intel/' + type + '/' + ip + '/' + action,
    method: 'POST',
    auth: {
      bearer: getToken()
    },
...
exports.submitException = function(action, exception) {
  if (!features.isOn("user:policy:submit")) {
    log.info("User policy submit feature is off");
    return;
  }

  log.info("Submit User Exception: '%s' => ", action, exception, {});

  if (!exception || !'i.type' in exception || !'p.dest.id' in exception) {
    log.warn('Invalid exception');
    return;
  }

  let type = exception['i.type'];
  let value = exception['p.dest.id'];
  switch (type) {
    case 'domain':
    case 'ip':
      break;
  ...
  if (!value) {
    log.warn('Invalid exception value: ' + value);
  }

  exports.intel(type, value, action, exception, (err) => {
  ...
  exports.submitUserPolicyAsync(policy)
    .then((obj) => {
      if (obj) {
        log.info("Submit user policy successfully");
      }
    }).catch((err) => {
    log.error("Submit user policy w/ error: ", err, {});
  });
...
exports.submitPolicy = function(action, policy) {
  exports.submitException(action, policy);
};

/*
function submitUserPolicy (policy, callback) {
  callback = callback || function() {};
  ...
  log.info("Pushing user policy to cloud: ", policy);
  let url = getEndpoint() + '/userpolicy';
  let options = {
    uri: url,
    method: 'POST',
    auth: {
...
exports.submitUserPolicyAsync = function(policy) {
  return new Promise((resolve, reject) => {
    submitUserPolicy(policy, (err, obj) => {
      if (err)
        reject(err);
      else
        resolve(obj);
