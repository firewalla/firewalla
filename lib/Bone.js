...
exports.checkinAsync = async function (boxVersion, license, info, useOriginalEndpoint = false) {
  ...
  if (['gold', 'goldpro', 'gse'].includes(platform.getName())) try {
  ...
  const response = await rrWithErrHandling(options, false, true)
  ...
  if (body) {
    ...
    if (body.jwt) {
      ...
      if (decoded) {
        ...
        log.warn("Invalid JWT in check-in response")
      }
    } else {
      log.warn("No JWT in check-in response, delete existing jwt")
...
exports.deviceAsync = async function (cmd, obj) {
  ...
  const response = await rrWithErrHandling(options, true, true)
...
exports.logAsync = async function (cmd, obj) {
  ...
  try {
    ...
    const response = await rrWithErrHandling(options, true, false, true);
    if (response.body) {
...
const intelFeedback = async function (feedback) {
  ...
  try {
    await rrWithErrHandling(options, true, true)
  } catch (err) {
    log.error(err)
  }
}
...
exports.intelAdvice = async (advice) => {
  ...
  try {
    await rrWithErrHandling(options, false, true)
  } catch (err) {
    log.error(err)
  }
