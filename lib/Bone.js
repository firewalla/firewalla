...
const diag = require('../extension/install/diag.js');
...
exports.checkinAsync = async function (config, license, info) {
  ...
  if (info.publicWanIps && info.publicWanIps.length > 0 || info.publicWanIp6s && info.publicWanIp6s.length > 0) {
    const ddns = {};
    if (info.publicWanIps && info.publicWanIp6s.length > 0) {
...
exports.logAsync = async function (cmd, obj) {
  ...
  if (!exports.cloudready()) {
    log.warn('Cloud not ready, using diag log');
    return diag.log(cmd, obj);
  }
