...
exports.submitUserIntel = function (action, intel) {
  ...
  let type = intel['i.type'] || intel['type'];
  let value = intel['p.dest.id'];
  switch (type) {
    case 'domain':
    case 'dns':
    case 'ip':
      value = value || intel['target_ip'];
      break;
    default:
  ...
  _value = querystring.escape(_value);

  exports.intel(type, _value, action, intel, (err) => {
    if (err) {
