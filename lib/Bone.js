...
const licenseModule = require('../util/license.js');
...
// boardcasting this doesn't really change jwt as it reads from sys:bone:jwt every 2s
...
// return new license on success, null on failure
async function rescueLicense() {
  const lastRequestStatus = await rclient.getAsync(Constants.REDIS_KEY_LAST_LICENSE_REQ_STATUS);
  if (lastRequestStatus && lastRequestStatus == Constants.LICENSE_REQ_STATUS_TRY_REFRESH) {
    // try to fetch a new license again
    log.info(`Last license request status was ${lastRequestStatus}, trying to fetch license again`);

    // to avoid looping forever, set the last license request status to TryLicenseRescue intermediately
    await rclient.setAsync(Constants.REDIS_KEY_LAST_LICENSE_REQ_STATUS, Constants.LICENSE_REQ_STATUS_TRY_RESCUE);
    const licenseJSON = await licenseModule.getLicenseAsync();
    const uuid = licenseJSON && licenseJSON.DATA && licenseJSON.DATA.UUID;
    const mac = await networkTool.getIdentifierMAC();
    if (!uuid || !mac) {
      log.info(`Cannot get UUID: ${uuid} or MAC address: ${mac} for license rescue`);
      return null;
    }
    await exports.waitUntilCloudReadyAsync();
    log.debug(`Attempting to get new license for rescue, UUID: ${uuid}, MAC: ${mac}`);

    const lic = await exports.getLicenseAsync(uuid, mac).catch((err) => {
      log.error("Failed to get new license after switching back to original endpoint:", err);
      return null;
    });

    if (lic) {
      const types = platform.getLicenseTypes();
      if (!lic.DATA || !lic.DATA.UUID ||
        !lic.DATA.LICENSE || lic.DATA.LICENSE.constructor.name !== 'String') {
        log.warn("New license data is invalid.");
      } else if (types && !types.includes(lic.DATA.LICENSE.toLowerCase())) {
        log.warn("New license type is not compatible with this device.");
      } else {
        log.forceInfo("Got a new license, switching back to original endpoint:", originalEndpoint);
        log.info("Got a new license:", lic.DATA.UUID.substring(0, 8));
        await licenseModule.writeLicense(lic);
...
exports.checkinAsync = async function (boxVersion, license, info, useOriginalEndpoint = false) {
  ...
  const osEpochTime = await exec(`date +%s`).then(result => Number(result.stdout.trim())).catch((err) => null);
  if (osEpochTime)
    obj.osEpochTime = osEpochTime;
  ...
  if (platform.isFireRouterManaged()) try {
    obj.fwapc = await fwapc.getCheckInData()
  } catch(err) {
    log.error('Error getting fwapc status')
  }

  if (fc.isFeatureOn("dap")) try {
    const sl = require('../sensor/SensorLoader.js');
    const dapSensor = sl.getSensor("DapSensor");
    if (dapSensor) {
      const dapCheckIn = await dapSensor.getCheckInData();
      if (dapCheckIn)
        obj.dapCheckIn = dapCheckIn;
    }
  } catch (err) {
    log.error('Error getting dap check-in data', err);
  }
  ...
  log.debug("Checking-In ...", boxVersion, license, JSON.stringify(obj));
  ...
  const response = await rrWithErrHandling(options).catch(async (err) => {

    if (err && typeof err.statusCode !== 'undefined' && err.statusCode >= 500 && endpoint != originalEndpoint) {
      log.info(`Check in to ${endpoint} return ${err.statusCode}`);
      
      newLicense = await rescueLicense().catch((err) => {
        log.error("Failed to rescue license:", err);
        return false;
      });

      if (newLicense) {
        // rescueLicense succeeded, try check-in again
        shouldRetry = true;
        return null;
      }
    } else {
  ...
  if (!response && shouldRetry && newLicense && _.isEqual(newLicense, license) == false) {
    return exports.checkinAsync(boxVersion, newLicense, info);
  }
  ...
  if (body) {
    ...
    if (body.jwt) {
      const decoded = tokenManager.verify(body.jwt);
      if (decoded) {
        jwt = body.jwt;
        expire = decoded.exp;
        publishJwt(jwt);
...
exports.deviceAsync = async function (cmd, obj) {
  log.verbose("sending POST request: /device/" + cmd);
  log.debug(obj)
  ...
  log.verbose("==== Device ===", response.body);
...
exports.hashsetAsync = async function (hashsetid) {
  ...
  // use connection pool for hash metadata
  const response = await rrWithErrHandling(options, hashsetid && hashsetid.startsWith("metadata:"));
...
exports.submitIntelFeedback = async function (action, intel) {
  ...
  log.verbose("Intel for feedback:", action, "=>", intel);
  ...
  if (!target) {
    log.error('Invalid target:', action, intel);
    return;
  }
...
exports.intelAdvice = async (advice) => {
  ...
  boneAPITimeSeries.recordHit('intelAdvice')
