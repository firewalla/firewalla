...
exports.checkinAsync = async function (config, license, info) {
  ...
  if (info.publicWanIps && info.publicWanIps.length > 0) { // publicWanIps in sysManager.getSysInfo returns sysManager.myConnectedPublicWanIps
    obj.ddns = {
      ip: info.ip && info.publicWanIps.includes(info.ip) ? info.ip : info.publicWanIps[0] // always register default WAN IP if it is a public IP
    };
  }
  if (license) {
