...
exports.alarmDecison = async (alarm) => {
  log.info(`Verifying alarm in cloud: ${alarm.aid}`);

  const options = {
    uri: getEndpoint() + '/alarm/decison',
    family: 4,
    method: 'POST',
    auth: {
      bearer: getToken()
  ...
  try {
    ...
    log.error(`Got error when calling alarm decison: ${err}`);
    alarm["p.cloud.error"] = "" + err;
    return alarm;
  }
}
