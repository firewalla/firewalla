...
if (firewalla.isProductionOrBeta() == false || _isOpaqueToDevCloud()) {
...
function _isOpaqueToDevCloud() {
  // alpha/beta user will be randomly selected to hit dev cloud.
  if (firewalla.isBeta() || firewalla.isAlpha()) {
    const random = Math.random();
    const devCloudOpacity = config.devCloudOpacity || 0.2;
    if (random < devCloudOpacity) {
      log.info("Randomly select beta cloud instance.")
      return true;
    }
  }
  return false;
...
exports.waitUntilCloudReadyAsync = Promise.promisify(waitUntilCloudReady);
...
exports.getLicenseAsync = Promise.promisify(exports.getLicense);
...
exports.checkin = function(config, license, info, callback) {
  ...
  rr(options, (err, httpResponse, body) => {
    if (err != null) {
      let stack = new Error().stack;
      log.info("Error while requesting checkin", err, stack);
      fallback();
      if (callback)
        callback(err, null, null);
      return;
    ...
    if (httpResponse == null) {
      let stack = new Error().stack;
      log.info("Error while response checkin", err, stack);
      fallback();
      if (callback)
        callback(500, null, null);
      return;
    ...
    if (httpResponse.statusCode < 200 ||
      httpResponse.statusCode > 299) {
      log.error("**** Error while response HTTP checkin", httpResponse.statusCode, body);
      fallback();
      if (callback)
        callback(httpResponse.statusCode, null, null);
      return;
...
exports.checkinAsync = Promise.promisify(exports.checkin);
...
exports.log = function(cmd, obj, callback) {
  ...
  request(options, (err, httpResponse, body) => {
    if (err != null) {
      let stack = new Error().stack;
      log.info("Error while requesting log", err, stack);
      if (callback)
        callback(err, null, null);
      return;
    }
    if (httpResponse == null) {
      let stack = new Error().stack;
      log.info("Error while response log", err, stack);
      if (callback)
        callback(500, null, null);
      return;
    }
    if (httpResponse.statusCode < 200 ||
      httpResponse.statusCode > 299) {
      log.error("**** Error while response HTTP log", httpResponse.statusCode);
      if (callback)
        callback(httpResponse.statusCode, null, null);
      return;
    }
...
exports.logAsync = Promise.promisify(exports.log);
