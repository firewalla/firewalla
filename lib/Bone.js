...
exports.checkinAsync = async function (boxVersion, license, info) {
  ...
  const ddns = {};
  const ddnsEnabled = await rclient.hgetAsync("policy:system", "ddns").then(result => JSON.parse(result).state !== false).catch((err) => true);
  if (!ddnsEnabled) {
    ddns.ip = "0.0.0.0";
  } else {
    // publicIp in sysManager.getSysInfo returns public IPv4 address from the selected outgoing WAN
    if (info.publicIp)
      ddns.ip = info.publicIp;
    else {
    ...
    if (_.isArray(info.publicIp6s) && !_.isEmpty(info.publicIp6s)) {
      ...
      // publicWanIp6s in sysManager.getSysInfo returns connected public IPv6 addresses
      if (info.publicWanIp6s && info.publicWanIp6s.length > 0) {
        const commonIp6s = (info.ip6 || []).filter(ip => info.publicWanIp6s.includes(ip));
        if (commonIp6s.length > 0)
          ddns.ipv6 = commonIp6s[0];
        else
          ddns.ipv6 = info.publicWanIp6s[0];
      }
    }
  }
  if (!_.isEmpty(ddns))
  ...
  if (body) {
    ...
    if (body.status == 302 && body.config && body.config.bone && body.config.bone.server) {
      if (endpoint != body.config.bone.server) {
        ...
        log.error("Check in: 302 on same endpoint", endpoint)
      }
    }
    if (body.jwt) {
      jwt = body.jwt;
