...
exports.checkinAsync = async function (boxVersion, license, info, useOriginalEndpoint = false) {
  ...
  if (body) {
    ...
    if (body.status >= 500 && endpoint != originalEndpoint) {
      log.info(`Check in return ${body.status} after redirecting to ${endpoint}`);
      // check last license request status
      try {
        ...
        if (lastRequestStatus && lastRequestStatus == Constants.LICENSE_REQ_STATUS_TRY_REFRESH) {
          ...
          const lic = exports.getLicenseAsync(uuid, mac).catch((err) => {
        ...
        log.error("Error during license rescue process:", err);
      }
