...
CONN_COLS='src_name:-30:u src_mac src_ip:-17 dst_name:-30:u dst_mac dst_ip:-17 port/prot:10 tx_bytes:10 rx_bytes:10 tx_count:10 rx_count:10'
(print_header; hl) >&2
lines=0
timeit begin

conns_data=$(local_api status/conntrack?format=json |\
    jq -r '.sources//empty|to_entries[]| .key as $sip |
           .value.destinations|to_entries[]|.key as $dip |
       .value.flowStats//empty |to_entries[] |
       [ $sip, $dip, .key,.value.txBytes,.value.rxBytes,.value.txCount,.value.rxCount ] | @tsv')

if true; then
    test -n "$conns_data" && echo "$conns_data" | while IFS=$'\t' read src_ip dst_ip port_protocol tx_bytes rx_bytes tx_count rx_count
    do
        ...
        do
            ...
            case $connc in
                ...
                dst_ip) connd=$dst_ip ;;
                port/prot) connd=$port_protocol ;;
                tx_bytes) connd=$tx_bytes ;;
                rx_bytes) connd=$rx_bytes ;;
                tx_count) connd=$tx_count ;;
                rx_count) connd=$rx_count ;;
                *) connd=$NO_VALUE ;;
            esac
