...
FWACMD="curl -s --max-time 5 -H 'Authorization: Bearer $(redis-cli hget sys:ept token)' 'https://ota.firewalla.com?type=box&model=${FIREWALLA_PLATFORM}&branch=${FW_BRANCH}' | jq '. | length' "
ratio=$(eval $FWACMD)
if [ "$ratio" == "0" ];
then
