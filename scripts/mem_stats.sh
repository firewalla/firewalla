...
REGEX="([A-Z][a-z]+ [0-9]+ [0-9]+:[0-9]+:[0-9]+).*FireApi Critical Memory Restart2 mon:([0-9]+),[[:space:]]*api:([0-9]+),[[:space:]]*main:([0-9]+),[[:space:]]*bro:([0-9]+),[[:space:]]*redis:([0-9]+),[[:space:]]*b7:([0-9]+),[[:space:]]*b6:([0-9]+),[[:space:]]*proc:([0-9]+),[[:space:]]*thread:([0-9]+)[[:space:]]*main_thread:[[:space:]]*([0-9]+)[[:space:]]*mon_threads:[[:space:]]*([0-9]+)[[:space:]]*mainfile:[[:space:]]*([0-9]+)[[:space:]]*monfile:([0-9]+)[[:space:]]*sys:(.*)"
...
sudo cat /var/log/syslog | grep 'Critical Memory Restart2' | while read LINE; do
  (if [[ $LINE =~ $REGEX ]]; then
