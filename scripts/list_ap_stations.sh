...
while true; do
    if [[ -z "$STATION_MAC" ]]; then
        sta_data=$(local_api status/station/flat| jq -r '.info[] |[.macAddr, .assetUID, .ssid, .band, .channel, .txnss, .rxnss, .rssi, .snr, .txRate, .rxRate, .curTxRate//"_", .curRxRate//"_", .intf, has("mlo"), .mlo.standby//"false", .assocTime, .ts, .idle, .dvlanVlanId, .vlanId]|@tsv')
    else 
        sta_data=$(local_api status/station/$STATION_MAC/flat| jq -r '.info[] |[.macAddr, .assetUID, .ssid, .band, .channel, .txnss, .rxnss, .rssi, .snr, .txRate, .rxRate, .curTxRate//"_", .curRxRate//"_", .intf, has("mlo"), .mlo.standby//"false", .assocTime, .ts, .idle, .dvlanVlanId, .vlanId]|@tsv')
    fi
    timeit sta-data
