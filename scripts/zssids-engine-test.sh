#!/bin/bash
#
# Tests for scripts/zssids-engine.sh.
#
#   ./scripts/zssids-engine-test.sh          sandbox only: renders the drop-ins
...
#   ./scripts/zssids-engine-test.sh --live   additionally exercises the live
...
export ZSSIDS_BIN=$T/zssids
export ZSSIDS_RUN_DIR=$T/assets
export ZSSIDS_ENGINE_LOCK=$T/apply.lock
export ZSSIDS_ENGINE_TEST_MODE=true
...
if $LIVE_CHECKS; then
  ...
  relive() {
    sudo -E env -u ZSSIDS_ENGINE_TEST_MODE -u SYSTEMD_DIR -u ZSSIDS_BIN -u FW_EFFECTIVE_FEATURES "$ENGINE" apply >/dev/null 2>&1 || true
...
cleanup() {
  if $LIVE_CHECKS; then
    ...
    sudo rm -f /dev/shm/zssids-engine.failed
    restore
    relive
  fi
...
B=$SYSTEMD_DIR/brofish.service.d/zssids.conf
S=$SYSTEMD_DIR/suricata.service.d/zssids.conf

echo "== zssids/zssids"
setf 1 1; "${SANDBOX[@]}" "$ENGINE" apply >/dev/null; rc=$?
check "apply returns 0" '[[ $rc -eq 0 ]]'
# the templates name the asset path; ZSSIDS_BIN here only stands in for its presence
ASSET=/home/pi/.firewalla/run/assets/zssids
# the brofish drop-in launches through the wrapper, the ids-only unit runs the
# binary directly
RUNNER=/home/pi/.firewalla/run/assets/zssids-run
IDS_RUNNER=/home/pi/.firewalla/run/assets/zssids-ids-run
LOCAL_RUNNER=$ZSSIDS_RUN_DIR/zssids-run
LOCAL_IDS_RUNNER=$ZSSIDS_RUN_DIR/zssids-ids-run
...
setf 1 1; rm -f "$ZSSIDS_BIN"; out=$("${SANDBOX[@]}" "$ENGINE" apply 2>&1); rc=$?
...
# pcap_zeek off with both zssids features on: the suricata unit must run zssids
...
echo "== restart starts a zssids-owned unit that is inactive"
# not run against the live units: check the code path instead
check "restart_zssids_services does not gate on is-active" '! grep -q "is-active -q brofish" "$ENGINE"'
check "restart_zssids_services resets a failed unit" 'grep -q "reset-failed brofish" "$ENGINE"'
...
if lsattr -d /etc/systemd/system/brofish.service.d 2>/dev/null | grep -q i; then
  # the live checks run against the box's real paths: the sandbox ZSSIDS_BIN
  ...
  out=$(sudo -E env -u ZSSIDS_ENGINE_TEST_MODE -u SYSTEMD_DIR -u ZSSIDS_BIN -u FW_EFFECTIVE_FEATURES "$ENGINE" apply 2>&1); rc=$?
  check "live apply fails" '[[ $rc -ne 0 ]]'
  check "hold marker is left behind" '[[ -e /dev/shm/zssids-engine.failed ]]'
  check "restart refuses while held" '! sudo -E env -u ZSSIDS_ENGINE_TEST_MODE -u SYSTEMD_DIR -u ZSSIDS_BIN -u FW_EFFECTIVE_FEATURES "$ENGINE" restart >/dev/null 2>&1'
  check "brofish was not restarted" '[[ "$(systemctl show brofish -p ExecStart --value)" == "$live_before" ]]'
  sudo chattr -i /etc/systemd/system/brofish.service.d 2>/dev/null
  sudo -E env -u ZSSIDS_ENGINE_TEST_MODE -u SYSTEMD_DIR -u ZSSIDS_BIN -u FW_EFFECTIVE_FEATURES "$ENGINE" apply >/dev/null 2>&1
  check "a successful live apply clears the marker" '[[ ! -e /dev/shm/zssids-engine.failed ]]'
  check "the live drop-in is back" '[[ -f /etc/systemd/system/brofish.service.d/zssids.conf ]]'
else
...
check "zssids-engine restart refuses while held" 'grep -q "not starting the pcap services" "$ENGINE"'
...
check "the roles share one process only when both are zssids and both enabled" 'sed -n "/^shared_roles()/,/^}/p" "$ENGINE" | grep -q "pcap_zeek_enabled && pcap_suricata_enabled"'
check "the shared process is not told to skip the rules" 'sed -n "/One process for both roles/,/^    fi/p" "$ENGINE" | grep -q "opts=[\"][\"]"'
check "the ids launcher takes suricata's interface list" 'grep -q "listen_interfaces.rc" "$FIREWALLA_HOME/scripts/zssids-ids-run"'
check "the ids launcher refuses an absent interface list" '! FIREWALLA_HIDDEN="$T/no-interfaces" ZSSIDS_BIN="$ZSSIDS_BIN" bash "$FIREWALLA_HOME/scripts/zssids-ids-run" >/dev/null 2>&1'
check "main-start guards the later zeekctl cron" 'grep -q "zssids-engine.failed" "$FIREWALLA_HOME/scripts/main-start"'
...
check "production rejects redirected privileged paths" '! env -u ZSSIDS_ENGINE_TEST_MODE SYSTEMD_DIR="$T/redirected" "$ENGINE" status >/dev/null 2>&1'
check "test mode never elevates scratch-path operations" 'grep -q "sudo() { command" "$ENGINE"'
check "the watchdog checks both roles" 'grep -q "ROLES+=" "$FIREWALLA_HOME/scripts/zssids-ping.sh"'
check "any apply failure is retried" 'grep -q "this.applyFailed" "$FIREWALLA_HOME/sensor/ZssidsEnginePlugin.js"'
check "ZssidsEnginePlugin also watches pcap_zeek / pcap_suricata" 'grep -q "FEATURE_PCAP_ZEEK," "$FIREWALLA_HOME/sensor/ZssidsEnginePlugin.js" && grep -q "FEATURE_PCAP_SURICATA" "$FIREWALLA_HOME/sensor/ZssidsEnginePlugin.js"'
check "the suricata watchdog is used whenever zssids owns the IDS" 'grep -q "appliedSuricataEngine() === .zssids." "$FIREWALLA_HOME/net2/SuricataControl.js"'
check "zssids-ping honours the hold" 'grep -q "held back, not checking" "$FIREWALLA_HOME/scripts/zssids-ping.sh"'
check "zssids-ping skips a role the box switched off" 'grep -q "pcap_zeek_enabled" "$FIREWALLA_HOME/scripts/zssids-ping.sh"'
...
roles=$(PATH=$T/bin:$PATH FIREWALLA_HIDDEN=$T/hidden-release FW_EFFECTIVE_FEATURES=/nonexistent bash -c "source \"$FIREWALLA_HOME/platform/platform.sh\"; branch=release_6_0; ZSSIDS_BIN=$ZSSIDS_BIN; echo \$(get_flow_engine_zeek)")
...
check "FireMain startup restarts when the applied state changed" 'grep -q "flow engine reconciled at startup" "$FIREWALLA_HOME/sensor/ZssidsEnginePlugin.js"'
...
for i in $(seq 1 25); do
  r=$(PATH=$T/bin:$PATH FW_EFFECTIVE_FEATURES=$eff bash -c "source \"$FIREWALLA_HOME/platform/platform.sh\"; ZSSIDS_BIN=$ZSSIDS_BIN; echo \$(get_flow_engine_zeek)")
  [[ $r == zssids || $r == zeek ]] || bad=$((bad+1))
done
wait $writer
check "every read answered a valid engine" '[[ $bad -eq 0 ]]'
check "node publishes the file atomically (temp + rename)" 'grep -q "renameSync" "$FIREWALLA_HOME/sensor/ZssidsEnginePlugin.js"'
...
unit() { # stub-body expected-rc name
  ...
  rc=$(ZSSIDS_ENGINE_SOURCE_ONLY=1 bash -c "
    FIREWALLA_HOME=$FIREWALLA_HOME
    source \"$ENGINE\" >/dev/null 2>&1
    $body
    ZEEK_ENGINE=zssids; SURICATA_ENGINE=zssids; BRO_PROC_NAME=zeek; ZSSIDS_BIN=$ZSSIDS_BIN
...
check "publishFeatures throws instead of logging" 'grep -q "throw new Error(\`publishing the effective flow engine features failed" "$FIREWALLA_HOME/sensor/ZssidsEnginePlugin.js"'

echo "== unit: verify accepts the wrapper for brofish"
check "verify checks for zssids-run under brofish" 'sed -n "/^verify()/,/^}/p" "$ENGINE" | grep -q "ZSSIDS_RUN"'

echo "== unit: zeek preparation and legacy cron"
check "the brofish drop-in launches through zssids-run" 'grep -q "^ExecStart=$RUNNER " "$FIREWALLA_HOME/etc/brofish-zssids.conf"'
check "zssids-run runs before_bro and after_bro" 'grep -q "before_bro ||" "$FIREWALLA_HOME/scripts/zssids-run" && grep -q "after_bro" "$FIREWALLA_HOME/scripts/zssids-run"'
check "the launcher copies are installed beside the asset" '[[ -x $LOCAL_RUNNER && -x $LOCAL_IDS_RUNNER ]]'
check "both drop-ins clear RemainAfterExit" 'grep -q "RemainAfterExit=false" "$FIREWALLA_HOME/etc/brofish-zssids.conf" && grep -q "RemainAfterExit=false" "$FIREWALLA_HOME/etc/suricata-zssids-ids.conf"'
check "the hourly zeekctl cron is removed while zssids owns the role" 'grep -q "cron.hourly/bro-cron" "$ENGINE"'
check "failure to remove the hourly zeekctl cron aborts apply" 'grep -q "fail .*removing /etc/cron.hourly/bro-cron" "$ENGINE"'
check "flow-check is skipped while the apply is held" 'grep -q "zssids-engine.failed || /home/pi/firewalla/scripts/flow-check.sh" "$FIREWALLA_HOME/etc/crontab.zssids"'
...
check "the feature listeners are registered before the initial apply" 'awk "/onFeature/{o=NR} /await this.apply\\(false\\)/{a=NR} END{exit !(o && a && o<a)}" "$FIREWALLA_HOME/sensor/ZssidsEnginePlugin.js"'

echo "== unit: the two-process arrangement"
check "the IDS unit is restarted only when it runs a zssids of its own" 'sed -n "/^restart_zssids_services()/,/^}/p" "$ENGINE" | grep -q "pcap_suricata_enabled && ! shared_roles"'
check "a leftover suricata process is stopped regardless of ExecStart" 'sed -n "/^stop_replaced_engines()/,/^}/p" "$ENGINE" | grep -q "SURICATA_ENGINE == zssids ]] && suricata_running; then"'
check "each cron entry checks one role" 'grep -q "zssids-ping.sh brofish" "$FIREWALLA_HOME/etc/crontab.zssids" && grep -q "zssids-ping.sh suricata" "$FIREWALLA_HOME/etc/suricata/crontab.zssids-ids"'
check "the watchdog probes the IDS with suricata interfaces" 'grep -q "ids_status_args" "$FIREWALLA_HOME/scripts/zssids-ping.sh"'
check "the retry flag is owned by apply()" 'grep -q "this.applyFailed = true;" "$FIREWALLA_HOME/sensor/ZssidsEnginePlugin.js"'
...
check "the drop-ins point outside the git checkout" 'grep -q "^ExecStart=$RUNNER " "$FIREWALLA_HOME/etc/brofish-zssids.conf" && grep -q "^ExecStart=$IDS_RUNNER " "$FIREWALLA_HOME/etc/suricata-zssids-ids.conf"'
check "apply refreshes the launcher copies" 'sed -n "/^apply()/,/^}/p" "$ENGINE" | grep -q "ZSSIDS_RUN_DIR/\$l"'
...
RC
chmod 755 "$T/bin/redis-cli"
roles=$(PATH=$T/bin:$PATH FW_EFFECTIVE_FEATURES=$T/hidden.json bash -c "source \"$FIREWALLA_HOME/platform/platform.sh\"; ZSSIDS_BIN=$ZSSIDS_BIN; echo \$(get_flow_engine_zeek)")
...
check "zssids-run queues a fresh stock-engine transaction" 'grep -q "systemctl --no-block restart" "$FIREWALLA_HOME/scripts/zssids-run" && ! grep -q "exec .*scripts/bro-run" "$FIREWALLA_HOME/scripts/zssids-run"'
check "zssids-ids-run queues a fresh stock-engine transaction" 'grep -q "systemctl --no-block restart" "$FIREWALLA_HOME/scripts/zssids-ids-run" && ! grep -q "exec .*scripts/suricata-run" "$FIREWALLA_HOME/scripts/zssids-ids-run"'
check "mandatory preparation failures abort startup" 'grep -q "before_bro failed" "$FIREWALLA_HOME/scripts/zssids-run" && grep -q "failed to apply" "$FIREWALLA_HOME/scripts/zssids-run"'
check "preparation runs in ExecStartPre, after_bro in ExecStartPost" 'grep -q "^ExecStartPre=$RUNNER --prepare" "$FIREWALLA_HOME/etc/brofish-zssids.conf" && grep -q "^ExecStartPost=$RUNNER --after-bro" "$FIREWALLA_HOME/etc/brofish-zssids.conf"'
...
setf 1 1
mkdir -p "$T/lock.d"
ZSSIDS_ENGINE_LOCK=$T/lock.d "${SANDBOX[@]}" ZSSIDS_ENGINE_LOCK=$T/lock.d "$ENGINE" apply >/dev/null 2>&1 & initializing=$!
sleep 1
...
rc=0
ZSSIDS_ENGINE_LOCK=$T/lock.d timeout 20 "${SANDBOX[@]}" ZSSIDS_ENGINE_LOCK=$T/lock.d "$ENGINE" apply >/dev/null 2>&1 || rc=$?
check "a live owner keeps the lock" '[[ $rc -ne 0 ]] && [[ -d $T/lock.d ]]'
kill $sleeper 2>/dev/null; wait $sleeper 2>/dev/null
ZSSIDS_ENGINE_LOCK=$T/lock.d "${SANDBOX[@]}" ZSSIDS_ENGINE_LOCK=$T/lock.d "$ENGINE" apply >/dev/null 2>&1
check "a dead owner's lock is taken over" '[[ ! -d $T/lock.d ]]'
rm -rf "$T/lock.d"

echo "== unit: one process only with a zssids that supports it"
check "shared mode asks the binary for the capability" 'sed -n "/^shared_roles()/,/^}/p" "$ENGINE" | grep -q "zssids_supports_shared_roles"'
check "the capability is read from the binary, not assumed" 'sed -n "/^zssids_supports_shared_roles()/,/^}/p" "$ENGINE" | grep -q -- "--capabilities"'
...
echo "== behaviour: a zssids without the capability keeps the two services apart"
cat > "$T/zssids" <<'OLD'
#!/bin/sh
# an older zssids: no --capabilities
exit 1
OLD
chmod 755 "$T/zssids"
setf 1 1
