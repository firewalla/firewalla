...
sudo iptables -t nat -A $CHAIN_NAME -d $FW_SS_SERVER -j RETURN
...
sudo iptables -t nat -A $CHAIN_NAME -d 198.51.100.99 -j RETURN

sudo iptables -t nat -A $CHAIN_NAME -p tcp -m set --match-set chnroute dst -j RETURN

sudo iptables -t nat -A $CHAIN_NAME -p tcp --destination-port 22:1023 -j REDIRECT --to-ports $FW_SS_LOCAL_PORT
