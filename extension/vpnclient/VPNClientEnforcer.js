...
class VPNClientEnforcer {
  ...
  async enforceStrictVPN(vpnIntf) {
    ...
    const cmd = wrapIptables(`sudo iptables -w -A FW_FORWARD -m mark --mark 0x${rtIdHex}/0xffff -m set ! --match-set trusted_ip_set dst ! -o ${vpnIntf} -j FW_DROP`);
  ...
  async unenforceStrictVPN(vpnIntf) {
    ...
    if (!rtId)
      return;
    const rtIdHex = Number(rtId).toString(16);
    const cmd = wrapIptables(`sudo iptables -w -D FW_FORWARD -m mark --mark 0x${rtIdHex}/0xffff -m set ! --match-set trusted_ip_set dst ! -o ${vpnIntf} -j FW_DROP`);
    await execAsync(cmd).catch((err) => {
