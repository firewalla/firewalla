...
class VPNClientEnforcer {
  ...
  async enforceDNSRedirect(intf, dnsServers) {
    if (!intf || !dnsServers || dnsServers.length == 0)
      return;
    const vpnClientIpset = this._getVPNClientIPSetName(intf);
    let cmd = `sudo ipset create -! ${vpnClientIpset} hash:ip family inet hashsize 128 maxelem 65536`
    await execAsync(cmd);
    for (let i in dnsServers) {
      ...
      if (i == 0) {
        ...
        cmd = wrapIptables(`sudo iptables -w -t nat -I PREROUTING -m set --match-set ${vpnClientIpset} src -p tcp --dport 53 -j DNAT --to-destination ${dnsServer}`);
        ...
        cmd = wrapIptables(`sudo iptables -w -t nat -I PREROUTING -m set --match-set ${vpnClientIpset} src -p tcp --dport 53 -m statistic --mode nth --every ${Number(i) + 1} --packet 0 -j DNAT --to-destination ${dnsServer}`);
        await execAsync(cmd).catch((err) => {
          log.error(`Failed to enforce DNS redirect rule: ${cmd}, intf: ${intf}, dnsServer: ${dnsServer}`, err);
        });
  ...
  async unenforceDNSRedirect(intf, dnsServers) {
    ...
    for (let i in dnsServers) {
      ...
      if (i == 0) {
        // no need to use statistic module for the first rule
        cmd = wrapIptables(`sudo iptables -w -t nat -D PREROUTING -m set --match-set ${vpnClientIpset} src -p tcp --dport 53 -j DNAT --to-destination ${dnsServer}`);
        await execAsync(cmd).catch((err) => {
          log.error(`Failed to enforce DNS redirect rule: ${cmd}, intf: ${intf}, dnsServer: ${dnsServer}`, err);
        });
        ...
        cmd = wrapIptables(`sudo iptables -w -t nat -D PREROUTING -m set --match-set ${vpnClientIpset} src -p tcp --dport 53 -m statistic --mode nth --every ${Number(i) + 1} --packet 0 -j DNAT --to-destination ${dnsServer}`);
        await execAsync(cmd).catch((err) => {
          log.error(`Failed to enforce DNS redirect rule: ${cmd}, intf: ${intf}, dnsServer: ${dnsServer}`, err);
        });
