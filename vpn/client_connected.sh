...
if [[ -n $CLIENT_SUBNETS ]]; then # CLIENT_SUBNETS are cidr subnets separated with comma
  ...
  do
    sudo ip r add $CLIENT_SUBNET ${PTP_ADDR:+via "$PTP_ADDR"} dev tun_fwvpn table lan_routable metric 1024 || true
    sudo ip r add $CLIENT_SUBNET ${PTP_ADDR:+via "$PTP_ADDR"} dev tun_fwvpn table wan_routable metric 1024 || true
    sudo ip r add $CLIENT_SUBNET ${PTP_ADDR:+via "$PTP_ADDR"} dev tun_fwvpn metric 1024 || true
  done
fi
