		...
		for (const vip of Object.keys(result)) {
			...
			if (!TableRow.length) {
				$("#vip_devices").append(
					...
					"<td id=\"vip_name\">Loading</td>" +
					"<td id=\"vip_ip\">Loading</td>" +
					"<td id=\"vip_latency\"><div class=\"progress\"><div style=\"width: 100%;\" class=\"progress-bar progress-bar-warning\"><small>Loading</small></div></div></td>" +
			...
			children["vip_name"].innerHTML = data.name;
			children["vip_ip"].innerHTML = vip;
			...
			if (stats && stats.length > 0) {
				...
				let latency_children = children["vip_latency"].children[0].children[0];
...
const _keys = ["name", "mac", "ip", "ap", "ssid", "channel", "rssi", "snr"];
const _vipKeys = ["vip_name", "vip_ip"];

//  sortTable(f,n)
//  f : 1 ascending order, -1 descending order
//  n : n-th child(<td>) of <tr>
function sortTable(f, n, key) {
	var rows = $(`#${key} tbody  tr`).get();
	...
	$.each(rows, function (index, row) {
		$(`#${key}`).children('tbody').append(row);
	});
}

function setSortState(x, asc, itemKey) {
	const resetKeys = itemKey === "sortKey" ? _keys : _vipKeys;
	...
	resetKeys.forEach((k) => {
		...
		if (text.endsWith(up)) {
			changed = true;
			text = text.replaceAll(up, "");
		}
		if (text.endsWith(down)) {
			changed = true;
			text = text.replaceAll(down, "");
		}
		if (changed)
			e.text(text);
	});
	...
	// set
	const e = $(`#${x}`);
	const state = asc === 1 ? up : down;
	e.text(`${e.text()}${state}`);
}

function mySort(itemKey) {
	const sortBy = localStorage.getItem(itemKey);
	const tableId = itemKey === "sortKey" ? "mytable" : "vip_table";
	if (sortBy) {
		const [key, asc] = sortBy.split(";");
		const n = $(`#${key}`).prevAll().length;
		sortTable(Number(asc), n, tableId);
		setSortState(key, Number(asc), itemKey);
	}
}

function applySort(x, itemKey) {
	const sortBy = localStorage.getItem(itemKey);
	if (sortBy) {
		let [key, asc] = sortBy.split(";");
		asc = key === x ? Number(asc) * -1 : 1;
		console.log("itemKey", itemKey);
		localStorage.setItem(itemKey, `${x};${asc}`);
	} else {
		localStorage.setItem(itemKey, `${x};1`);
	}

	mySort(itemKey);
}

function registerSortCallbacks() {
	_keys.forEach((x) => {
		$(`#${x}`).click(() => applySort(x, "sortKey"));
	});
	_vipKeys.forEach((x) => {
		$(`#${x}`).click(() => applySort(x, "vipSortKey"));
	});
}

registerSortCallbacks();
