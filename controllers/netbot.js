  ...
  async cmdHandler(gid, msg) {
    ...
    switch (msg.data.item) {
      ...
      case "rekey":
        ...
        return
      case "enableIv": {
        // Enable the per-request IV scheme for the group by setting the "iv"
        // marker in the group info via the cloud update-group API. No-op if set.
        const ivVersion = (value && value.iv) || 1;
        return await this.eptcloud.enableGroupInfoIV(gid, ivVersion);
      }
      case "disableIv": {
      ...
      case "policy:reset": {
        ...
        // setting firerouter config takes a few seconds, run it in parallel with the whole routine
        await Promise.all([
          value.route && FireRouter.removeStaticRoutes(), // false is not a promise so && is safe here
          (async () => {
            ...
            if (value.audit) {
              ...
              const pFeature = _.groupBy(pAllowBlock, p =>
                p.purpose && (['port_forwarding', 'dmz'].includes(p.purpose) || p.purpose.startsWith('vpn_'))
                || false
              )
